...
- **Locally restyled Alliance channel lines** - every chat message received on the `Alliance` channel is displayed with:
  - A yellow channel tag rendered as **[Alliance]**
...
- Receivers strip that hidden Alliance payload locally and still render the channel tag as `[Alliance]`.
...
3. Re-hooks `ChatFrame_MessageEventHandler` after login and on channel UI refresh so later-loading chat addons do not take back the channel renderer, then:
   ...
   - Intercept `CHAT_MSG_CHANNEL` events for the `Alliance` channel, strip that hidden payload, and re-print a locally styled replacement with **[Alliance]** followed by yellow linked sender/message text.
